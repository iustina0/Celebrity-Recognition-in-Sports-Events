...
from FaceDetection.data.dataset import TrainDataset, TestDataset
from FaceDetection.model.fasterRCNN import FasterRCNN
from torch.utils.data import DataLoader
from FaceDetection.model.trainer import FasterRCNNTrainer
from FaceDetection.utils.array_util import scalar
from FaceDetection.utils.eval_tool import eval_detection
...
def eval(dataloader, faster_rcnn, test_num=1000):
    ...
    for ii, (imgs, sizes, gt_bboxes_, gt_labels_) in tqdm(enumerate(dataloader), total=test_num, desc='Evaluation Progress'):
...
def train():
    ...
    if opt.load_path:
        ...
        print(f'Loaded pretrained model from {opt.load_path}')
    ...
    for epoch in range(opt.epoch):
        ...
        for ii, (img, bbox_, label_, scale) in tqdm(enumerate(dataloader), total=len(dataloader), desc=f'Training Epoch {epoch+1}/{opt.epoch}'):
            scale = scalar(scale)
        ...
        with open('./log.txt', 'w') as f:
            f.write(f'total_loss: {total_loss}, test_map: {test_map}, loss_values: {loss_values}')

            
        print(f'Epoch {epoch+1}/{opt.epoch}: current map: {eval_result["map"]}, best map: {best_map}')
    ...
    print(f'Test map: {test_map} Total loss: {total_loss} Loss values: {loss_values}')

    plt.figure()
    plt.plot(test_map, label='test_map')
    plt.legend()
    plt.savefig('./FaceDetection/misc/test_map_plot.png')

    plt.figure()
    plt.plot(total_loss, label='total_loss')
    plt.legend()
    plt.savefig('./FaceDetection/misc/total_loss_plot.png')

    plt.figure()
    for loss_name, values in loss_values.items():
        plt.plot(values, label=loss_name)
    plt.legend()
    plt.savefig('./FaceDetection/misc/loss_values_plot.png')
